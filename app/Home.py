...
def _build_epidemic_report(
    ...
    L += ["",
          "*Report generated by **EpiSurveil Control Platform** using Bootstrap Particle Filter.*  ",
...
# ---------------------------------------------------------------------------
with tab_comp:
    st.subheader("Filtered compartment trajectories: posterior mean with 80% CI")
    st.caption(
...
# ---------------------------------------------------------------------------
with tab_mod:
    st.subheader("SVEAIHCRD model: equations, data and control")
    st.caption(
    ...
    st.markdown(
        ...
        r"**$Q_{C,t}$, the 5th dynamic parameter**: log-random-walk bounded $[0.20,\,0.99]$, prior mean 0.40 in "
    ...
    st.caption(
        "Note on 'stochastic' vs. 'deterministic' MPC: running a fully stochastic MPC "
        "(where the particle cloud is propagated *inside* the optimiser at every function evaluation) would cost "
...
with tab_ctrl:
    ...
    st.caption(
        ...
        "u*(t): NPI intensity (reduces contact rate); v*(t): testing intensity (detected positives self-isolate)."
    )
    ...
    with mpc_col_out:
        ...
        if "mpc_result" in st.session_state:
            ...
            st.caption(
                "The controller re-solves every day using the BPF posterior mean; when "
...
| N_live | S + E + I + R |
| γ | Recovery rate: 0.10 day⁻¹ (10-day infectious period) |
...
Added term: **(1 − ε) · V_t**, the residual susceptibility of vaccinated individuals
...
# ===========================================================================
with tab_geo:
    st.subheader("Country comparison: R_eff and epidemic status across countries")
    st.caption(
